CREATE OR REPLACE FUNCTION audit_column_to_query(
	...
	_alter VARCHAR DEFAULT ''
)
	RETURNS VARCHAR
	AS $func$
BEGIN
	IF data_type = 'USER-DEFINED' THEN
		RETURN CONCAT(' "', column_name, '" ', _alter,' VARCHAR ');
	END IF;

	IF udt_name = 'varchar' AND character_maximum_length IS NOT NULL THEN
		RETURN CONCAT(column_name, '" ', _alter ,' VARCHAR(', character_maximum_length ,') ');
	END IF;

	RETURN CONCAT(' "', column_name, '" ', _alter, ' ', udt_name, ' ');
END
$func$
LANGUAGE plpgsql;